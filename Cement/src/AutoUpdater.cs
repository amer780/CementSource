...
public class AutoUpdater
{
    ...
    public readonly bool melonHasValidDownloadLink;

    public AutoUpdater(MelonMod itemToUpdate)
    {
        _melonToUpdate = itemToUpdate;

        melonHasValidDownloadLink = Uri.TryCreate(_melonToUpdate.Info.DownloadLink, UriKind.Absolute, out _melonReleaseUri) 
    ...
    public async Task UpdateItem()
    {
        ...
        try
        {
            ...
            var releaseResponseObj = JsonNode.Parse(releaseResponseBody)?.AsObject();
            if (releaseResponseObj is null) throw new NullReferenceException("Response did not come back, or came back as a non-json value/null. Update will not continue.");
            ...
            if (releaseResponseObj.TryGetPropertyValueAs("message", out string? message) && message == "Not Found") throw new Exception("Response came back with 'Not Found' message. This likely means you have the right host name (https://api.github.com), but the repo you requested to doesn't exist/is private. Update will not continue.");

            var releaseName = releaseResponseObj["name"]?.AsValue();
            if (releaseName is null || releaseName.GetValue<string>() is null) throw new NullReferenceException($"Release name is null somehow! Update will not continue.");
            
            Melon<Mod>.Logger.Msg($"Found release name: {releaseName}");

            var releaseTag = releaseResponseObj["tag_name"]?.AsValue();
            if (releaseTag is null || releaseTag.GetValue<string>() is null) throw new NullReferenceException($"Release has no tag name, making version checking impossible. Update will not continue.");
            if (!Version.TryParse(releaseTag.GetValue<string>(), out var releaseVersion) || releaseVersion is null) throw new NullReferenceException($"Failed to parse tag name of release {releaseName} as type {nameof(Version)}. Update will not continue.");
            ...
            var assetsResponseObj = releaseResponseObj["assets"]?.AsArray();
            if (assetsResponseObj is null) throw new NullReferenceException("GitHub release does not have an assets property. Update will not continue.");
        ...
        // Restart game
        Melon<Mod>.Logger.Msg("Updates were made, rebooting game. . .");
        Application.Quit();
